...
try:
    ...
    kakao_key = "발급받은_API키를_입력하세요"  # 기본값
...
if not spaces_df.empty:
    ...
    <script>
        ...
        var positions = {json.dumps(spaces_df.to_dict('records'))};
        ...
        positions.forEach(function(position) {{
            ...
            var infowindow = new kakao.maps.InfoWindow({{
                content: '<div style="padding:5px;width:200px;"><strong>' + position.name + 
                         '</strong><br>수용인원: ' + position.capacity + '명<br>' +
    ...
    # 공간 정보 표시
    st.subheader("서초문화원 강당")
    st.write("주소: 서울특별시 서초구 반포동 234-5")
    st.write("수용인원: 100명")
    st.write("운영시간: 10:00 - 20:00 (월-토)")
    st.write("특징: 무료대여, 지하철역 도보 5분")
    
    # 실제 데이터가 있으면 표시
    if len(spaces_df) > 1:
        st.divider()
        for _, space in spaces_df.iterrows():
            if space['name'] != "서초문화원 강당":  # 중복 방지
                with st.container():
